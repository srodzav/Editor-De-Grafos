...
namespace EditordeGrafos
{
    public partial class Editor : Form
    {
            ...
            switch (option)
            {
                ...
                case 3:
                    ...
                    if (des != null && nu != null)
                    {
                        ...
                        string edgename = graph.getMissingOrActualEdgeName();
                        if (edgename == "")
                        {
                            ed = new Edge(0, nu, des, "e" + graph.EdgesList.Count.ToString());
                        }
                        else
                        {
                            ed = new Edge(0, nu, des, edgename);
                        }
                        ed.Weight = obtenPeso();
                        graph.AddEdge(ed);
                        graph.SortEdgeListByName();
        ...
        private int obtenPeso()
        { // FUNCION QUE PERMITE OBTENER EL PESO, ESTE SE HABILITA/DESHABILITA
            int peso;
            string input = "";
            if (graph.EdgeWeightVisible) // SI LA BANDERA ESTA ACTIVA
            {
                input = Microsoft.VisualBasic.Interaction.InputBox("Introduzca el peso de la arista: ", "Peso de la Arista", "", Top, Left); // LECTURA DEL PESO
                while (!int.TryParse(input, out peso))
                { // SI EL USUARIO SE REHUSA A METER EL PESO, ESTE ENTRA EN UN WHILE 
                    input = Microsoft.VisualBasic.Interaction.InputBox("Vuelva a introducir el peso de la arista, asegurese que sea un número: ", "Peso de la Arista", "", Top, Left);
                }
            }
            else
            {
        ...
        */
        ...
        {
            graph.bpfrecorrido(1);
        }

        private void tT_EnablePeso_Click(object sender, EventArgs e)
        {
            if (graph.EdgeWeightVisible)
            {
                graph.EdgeWeightVisible = false;
            }
            else
            {
        ...
        #region RECORRIDOS EN TOOLBAR (ANTES DE AGREGAR BOTONES)
        private void bpfToolStripMenuItem_Click(object sender, EventArgs e)
        {
            graph.bpfrecorrido(1);
        }

        private void beaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            graph.bearecorrido();
        }
        ...
        {
            graph.bpfrecorrido(2);
        }

        #endregion

        #region EULER
        private void Euler(object sender, EventArgs e)
        { // ALGORITMO EULERIANO
            List<Edge> eu = new List<Edge>(); // ARISTAS EULERIANAS
            List<NodeP> nE = new List<NodeP>(); // NODOS EULERIANOS
            NodeP d;
            string c;

            graph.UnselectAllEdges();
            if (VerifyCircuitEuler())
            {
                EulerCircuit(eu, graph.First(), graph.First(), false, nE);
                c = PrintListNode(nE, "");
                MessageBox.Show("Circuito Euleriano: \n" + c);
            }
            else
            {
                if (VerifyPathEuler())
                {
                    ...
                    EulerPath(eu, d, nE);
                    c = PrintListNode(nE, "");
                    MessageBox.Show("Camino Euleriano: \n" + c);
                }
                else
                {
        ...
        private bool EulerCircuit(List<Edge> circuito, NodeP OG, NodeP w, bool foundOG, List<NodeP> nodosRecorridos)
        {
            Edge road = new Edge();
            nodosRecorridos.Add(w);
            foreach (NodeR r in w.relations)
            {
                road = graph.GetEdge(w, r.Up);
                if (!road.Visited)
                {
                    road.Visited = true;
                    circuito.Add(road);
                    foundOG = EulerCircuit(circuito, OG, r.Up, foundOG, nodosRecorridos);
        ...
        private void EulerPath(List<Edge> circuito, NodeP w, List<NodeP> nodosRecorridos)
        {
            Edge road = new Edge();
            nodosRecorridos.Add(w);
            foreach (NodeR r in w.relations)
            {
                ...
                if (!road.Visited)
                {
                    road.Visited = true;
                    circuito.Add(road);
                    EulerPath(circuito, r.Up, nodosRecorridos);
                    if (circuito.Count != graph.edgesList.Count)
                    {
                        circuito.Remove(road);
                        nodosRecorridos.RemoveAt(nodosRecorridos.Count - 1);
        ...
        private bool VerifyCircuitEuler()
        {
            bool circuitoE = true;
        ...
        private void CastDijkstra(object sender, EventArgs e)
        {
            SHOWdijkstra d = new SHOWdijkstra(graph);
            d.ShowDialog();
        }
        ...
        private List<Edge> Prim()
        {
            if (graph.IsConnectedU())
            {
                ...
                minEdge.Source.Visited = minEdge.Destiny.Visited = true;
                ...
                while (!graph.AllNodesVisited())
                {
                    ...
                    foreach (Edge ed in graph.EdgesList)
                    {
                        ...
                        if (((ed.Destiny.Visited == true && ed.Source.Visited == false) || (ed.Destiny.Visited == false && ed.Source.Visited == true)) && !ed.Visited)
                        {
                            if (ed.Weight < minValue)
                            {
                                minValue = ed.Weight;
        ...
        private void btn_bpf_Click(object sender, EventArgs e)
        {
            graph.bpfrecorrido(1);
        }

        private void btn_bea_Click(object sender, EventArgs e)
        {
            graph.bearecorrido();
        }

        private void btn_bap_Click(object sender, EventArgs e)
        {
            graph.bpfrecorrido(2);
        }
        ...
        private void btn_app_Click(object sender, EventArgs e)
        {
            ...
            SHOWapp d = new SHOWapp(graph);
            d.ShowDialog();
        }
        #endregion

        #region ARBOLES EN CONSOLA
        private void btn_arbol_Click(object sender, EventArgs e)
        {
            SHOWarbolb d = new SHOWarbolb();
        }
        #endregion
    }
}
